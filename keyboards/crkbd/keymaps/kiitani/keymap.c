...
int layer_state_my = 0;

void oled_render_layer_state(void) {
    oled_write_P(PSTR("Layer: "), false);
    switch (layer_state_my) {
        case L_BASE:
  ...
  switch (keycode) {
    case LOWER:
      if (record->event.pressed) {
        ...
        layer_state_my = L_LOWER;
      } else {
        layer_off(L_LOWER);
        update_tri_layer(L_BASE, L_RAISE, L_ADJUST);
        layer_state_my = L_BASE;
